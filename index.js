...
async function sendToDiscord(message, DISCORD_WEBHOOK_URL) {
  ...
  let embedBody = JSON.stringify({
    embeds: [
      {
        title: trimToLimit(email.subject, 256), // Limit is 256
        description:
          emailText.length > DISCORD_EMBED_LIMIT
            ...
            : emailText,
        author: {
          name: `${(trimToLimit(email.from.name, 100))}${email.from.name.length > 64 ? "\n" : " "}<${trimToLimit(email.from.address, 100)}>`, // Limit of 256 characters, but we will be a bit careful
        },
        footer: {
          text: `This email was sent to ${trimToLimit(message.to, 100)}\nEnvelope From: ${trimToLimit(message.from, 100)}`, // Limit of 2048 characters, but we will be careful
        },
      },
    ],
  });
  ...
  let discordResponse = await fetch(DISCORD_WEBHOOK_URL, {
    method: "POST",
    body: formData,
  });
